...
export async function getRecommendedUsers(req, res) {
  try {
    ...
    const recommendedUsers = await User.find({
      $and: [
        { _id: { $ne: currentUserId } }, //exclude user
        { _id: { $nin: currentUser.friends } }, //exlcude current user friends
        { isOnboarded: true },
      ],
    });
...
export async function getMyFriends(req, res) {
  try {
    ...
    console.error("Error in getMyFruends Controller", error.message);
    res.status(500).json({ message: "Internal Server Error" });
  }
}
...
export async function sendFriendRequest(req, res) {
  try {
    ...
    // prevent to send request to your self
    if (myId === recipientId) {
      return res
        .status(400)
    ...
    if (existingRequest) {
      return res.status(400).json({
        message: "A friend request already exists betwee you and this user",
      });
    }
...
export async function acceptFriendRequest(req, res) {
  try {
    ...
    //$addtoset: adds value to array only if it dosen't exist
